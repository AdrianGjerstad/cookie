...
const lest::test specification[] = {
  CASE( "Token retrieves strings properly" ) {
    ...
    cookie::Position pstart(1, 1, 0, "cookiec/test/data/example.cookie",
        source);
  ...
  CASE( "Token retrieves numbers (i32) properly" ) {
    ...
    for (unsigned int test_i = 0; test_i < ITERATIONS; ++test_i) {
      ...
      cookie::Position pend(1, source->size()+1, source->size(), "code",
          source);
      cookie::Token token(pstart, pend, cookie::TokenType::INT32);
  ...
  CASE( "Token retrieves numbers (i64) properly" ) {
    ...
    std::default_random_engine generator;
    std::uniform_int_distribution<int64_t> distribution(-655360000, 655350000);

    for (unsigned int test_i = 0; test_i < ITERATIONS; ++test_i) {
      const int64_t expected = distribution(generator);
      std::string* source = new std::string(std::to_string(expected));
      pool.add_hardcode("code", source);
  ...
  CASE( "Token retrieves numbers (u64) properly" ) {
    ...
    std::default_random_engine generator;
    std::uniform_int_distribution<uint64_t> distribution(0, 655350000);

    for (unsigned int test_i = 0; test_i < ITERATIONS; ++test_i) {
      const uint64_t expected = distribution(generator);
      std::string* source = new std::string(std::to_string(expected));
      pool.add_hardcode("code", source);
